...
class StrategyEmber(BaseStrategy):
    ...
    7. REJECT if entry is into prior swing-low support without a clear 5m CLOSE through that level.
       A volume spike at the floor is often absorption, not cascade fuel.
...
5. Entry is NOT sitting on prior swing support — require a clear 5m close-through (volume spike does NOT override)
...
- Entry pressed into prior swing-low support without a clear 5m close breakdown
...
"""
    ...
    def _build_sl_tp(
        ...
        closed = closed_bars(df_ctx)
        try:
            swing_high = float(closed["high"].iloc[-lookback:].max())
        except Exception:
            swing_high = entry
    ...
    def generate_signal(self, df, extra_data=None):
        ...
        active, cascade = detect_ember(df_5m, use_live=CASCADE_ENTRY_USE_LIVE)
        if not active:
            self.looking_for_entry = False
            self.entry_direction = None
        ...
        try:
            rsi_5m = float(df_5m["RSI_14"].iloc[rsi_idx])
        except Exception:
            rsi_5m = 50.0
        ...
        vol_reason = cascade_volume_reject_reason(
            df_5m, p, live_cascade=CASCADE_ENTRY_USE_LIVE
        )
        if vol_reason:
            self.looking_for_entry = False
        ...
        try:
            swing_high = float(
                closed_bars(df_5m)["high"].iloc[-int(p["sl_swing_lookback"]) :].max()
            )
        except Exception:
            swing_high = sl
