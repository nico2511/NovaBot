...
class StrategyRocket(BaseStrategy):
    ...
    8. REJECT if entry is into prior swing-high resistance without a clear 15m CLOSE through that level.
       A volume spike at the ceiling is often absorption (sellers defending), not cascade fuel.
       A 1m wick/pierce through resistance is not a breakout.
...
5. Entry is NOT sitting on prior swing resistance — require a clear 15m close-through (volume spike does NOT override)
...
- Entry pressed into prior swing-high resistance without a clear 15m close breakout
...
"""
    ...
    def generate_signal(self, df, extra_data=None):
        ...
        geo_reason = self.geometry_reject_reason(
            {"signal": "BUY", "price": float(entry), "sl": float(sl), "tp": float(tp)},
            df_15m,
        )
        if geo_reason:
            self.looking_for_entry = False
    ...
    def evaluate_trade_thesis(self, trade, current_price, *, df, extra_data=None):
        ...
        entry_veto_rsi = self._float_param("veto_rsi_overbought", 72.0)
