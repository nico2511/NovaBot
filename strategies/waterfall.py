...
The strategy already confirmed: closed 15m bearish cascade. The last closed
1m must still be red and must not have run away from that 15m close.
A 1m lower-low is NOT required. Fast sanity check only.
...
"""
    ...
    def generate_signal(self, df, extra_data=None):
        ...
        if status != "enter" or entry is None:
            self.looking_for_entry = False
            return self._reject(minute_reason or "late waterfall — 1m chased the confirmed close")
        ...
        try:
            swing_high = float(
                closed_bars(df_15m)["high"].iloc[-int(p["sl_swing_lookback"]) :].max()
            )
        except Exception:
            swing_high = sl
        ...
        return {
            ...
            "comment": (
                f"Waterfall: confirmed 15m cascade (price < EMA9 < EMA20, double red, LL). "
                f"Entry {entry:.6g} (1m with-trend, no extra LL), "
                f"SL {sl_pct:.2f}% above swing, R:R {rr:.2f}"
            ),
        }
