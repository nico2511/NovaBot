#!/usr/bin/env python3
"""
Backtest for Today (December 26, 2025)
Tests all active strategies on today's market data
"""
import pandas as pd
...
def run_today_backtest(symbol="BTC"):
    print(f"🔄 Fetching today's data for {symbol}...")
    print(f"📅 Date: {datetime.now().strftime('%Y-%m-%d')}")
    
    # Fetch 15m and 1m data for today
    # 1 day = 96 x 15m candles, fetch 200 for buffer
    df_15m = hyperliquid_service.get_candles(symbol, interval="15m", limit=200)
    df_1m = hyperliquid_service.get_candles(symbol, interval="1m", limit=1500)  # ~1 day of 1m candles
    ...
    df_15m = df_15m[df_15m.index.date == today]
    df_1m = df_1m[df_1m.index.date == today]
    
    print(f"✅ Loaded {len(df_15m)} x 15m candles and {len(df_1m)} x 1m candles for today")
    ...
    min_window = 50  # Minimum candles for indicators
    ...
    for i in range(min_window, len(df_15m)):
        ...
        # Get corresponding 1m data up to this point
    ...
    # Report
    print("\n" + "="*60)
    print(f"📊 BACKTEST REPORT - {symbol} - {datetime.now().strftime('%Y-%m-%d')}")
    print("="*60)
    ...
    print(f"Period: Today ({datetime.now().strftime('%Y-%m-%d')})")
