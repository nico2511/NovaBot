...
description: >-
  ...
  persona, hard veto, scan ranking, post_ai_adjust, generate_signal). Use when
  adding a new strategy, scaffolding from _template_strategy.py, registering in
  engine.py / strategies.json, writing strategy unit tests, or when the user
  mentions strategies/README.md, BaseStrategy, Trend LT, SuperTrend, scanner,
  or "nouvelle stratégie".
---
...
- [ ] 4. Register in strategies/engine.py
- [ ] 5. Add JSON to data/config/strategies.json AND app/core/defaults/strategies.default.json
- [ ] 6. Unit tests (persona/criteria, veto, signal reject, scan score if scannable)
- [ ] 7. No métier if strategy_id == "..." in bot.py / ia.py / scanner_job.py
- [ ] 8. Params only via get_param(); sticky fields if multi-symbol armed state needed
- [ ] 9. If non-15m TF: set timeframe (drives scan TF + skips engine 15m BB anti-chase)
```
